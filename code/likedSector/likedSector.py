...
async def likedSector(sector = str):
    ...
    userMessageRaw =  "당신은 이제부터 '최근 며칠간의 기사 목록'에 대한 dict를 python dictionary로 입력받을 것입니다. \
                    #최근 며칠간의 기사 목록\
                    - {info}\
                    기사 목록에 대한 dictionary는 {{기사 제목 : 기사 작성 시간}}의 형태로 구성되어 있으며 기사를 참고하여 \
                    '관심종목'의 전망에 대한 당신의 추론과 향후 7일간 가장 주가가 많이 상승할 것 같은 회사 이름 7개를 적고, \
    ...
    userMessage =  "- 주어진 문장 : {gptAnswer}\
                    ...
                    정해진 형식에 맞게 주어진 문장에서 관심종목과, 선택한 기업과 이유 7개를 추려내어 json데이터로 변환하세요.\
                    기업이름1~7에는 기업의 이름이, 선택이유1~7에는 선택한 이유가 각각 들어가야 합니다.\

...
async def likedSector(sector = str):
    ...
    userMessageRaw =  "### Instruction & Inputs###\
                    당신은 이제부터 '최근 며칠간의 기사 목록'에 대한 dict를 python dictionary로 입력받을 것입니다. \
                    #최근 며칠간의 기사 목록\
                    - {info}\
                    \
                    ### Notes ###\
                    기사 목록에 대한 dictionary는 {{기사 제목 : 기사 작성 시간}}의 형태로 구성되어 있으며 \
                    최근의 사건들을 참고하여 '{sectors1}'분야 기업에서 향후 1개월간 주가가 상승할 것 같은 국내 기업 6가지 당신의 추론을 적되, \
                    기업의 시가총액이 작은 것부터 4가지, 큰 것부터 2가지를 적어 주세요.\
                    json 형태로 답변하고 그렇게 생각한 이유는 최대한 구체적이게 작성해 주세요.\
                    \
                    ### Response Format ###\
                    - [ ['회사1', '추론근거1'],\
                        ['회사2', '추론근거2'],\
                    ...
                    \
                    if '관심종목' == 게임\
                    - [['크래프톤', '크래프톤은 국내 게임 시장에서 지속적으로 선전하고 있으며, 신규 서비스 출시로 인해 수익성이 개선되고 있는 추세다. 게임 시장 성장을 고려할 때, 주가가 7% 이상 증가할 것으로 예상된다.'],\
                    ...
                    ### Response ###\
                    #당신의 추론('관심종목' == {sectors2})\
                    ".format(info = gpt_insert_info["dict"], sectors1 = gpt_insert_info["item"], sectors2 = gpt_insert_info["item"])
    ...
    userMessage =  "- 주어진 문장 : {gptAnswer}\
                    ...
                    정해진 형식에 맞게 주어진 문장에서 선택한 기업과 이유 6개를 추려내어 json데이터로 변환하세요.\
                    기업이름1~6에는 기업의 이름이, 선택이유1~6에는 선택한 이유가 각각 들어가야 하고 반드시 문장으로 끝나야 합니다. '~~해서'의 형태는 금지되고 항상 '~~입니다' 의 형식으로 출력하세요.\
...
@router.get("/stockPrice/{name}", summary="관심 회사 주가")
async def priceList(name: str):
    """
    관심종목으로 설정한 회사의 주가를 반환하는 엔드포인트입니다.

    """
    price = [50000, 49900, 49700, 49400, 49300, 49200, 49400, 49400, 49300, 49300, 
49300, 49300, 49100, 49000, 49200, 49500, 49200, 49100, 49200, 49200, 
49200, 49100, 49200, 49500, 49200, 49400, 49400, 49600, 49400, 49500, 
49300, 49000, 49000, 48800, 48600, 48800, 48800, 48800, 49000, 48700, 
48700, 48900, 48600, 48600, 48900, 49100, 48800, 48700, 48900, 48900, 
49100, 49100, 49000, 49300, 49100, 49100, 49100, 49300, 49600, 49800, 
50000, 50300, 50500, 50300, 50200, 50300, 50600, 50300, 50100, 50000, 
49900, 50200, 50100, 49900, 49600, 49400, 49200, 49500, 49600, 49400, 
49400, 49500, 49700, 50000, 50000, 49700, 49600, 49700, 50000, 49900]
